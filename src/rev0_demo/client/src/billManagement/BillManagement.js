  ...
  return (
    <Layout>
      <div className="container">
        ...
        <h1 className="mt-3">
          {updatedUserAmount < 0 ? `Overall, you owe $${Math.abs(updatedUserAmount).toFixed(2)}` : `Overall, you are owed $${updatedUserAmount.toFixed(2)}`}
        </h1>
        ...
        <div className="row row-cols-1 row-cols-md-2 g-4">
          {Object.entries(debts).filter(([user, amount]) => user !== currentUser.email).map(([user, amount]) => (
            <div key={user} className="col">
              <div className={`card ${amount < 0 ? "bg-danger" : "bg-success"} text-white`}>
          ...
          <div className="modal fade show" tabIndex="-1" role="dialog" style={{ display: 'block' }}>
            <div className="modal-backdrop fade show"></div>
              <div className="modal-dialog modal-dialog-centered" role="document" style={{ zIndex: 1050 }}>
                <div className="modal-content">
                  <div className="modal-header">
                    ...
                    <button type="button" className="close" data-dismiss="modal" aria-label="Close" onClick={() => setShowSettlementModal(false)}>
              ...
              <div className="modal-footer">
                ...
                <button type="button" className="btn btn-success" onClick={handleSettleExpense} disabled={settlementAmount > Math.abs(debts[settlingDebt])}>Settle Expense</button>
              </div>
            </div>
          </div>
        </div>
